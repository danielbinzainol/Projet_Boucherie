...
import { Minus, Plus, Trash2, ReceiptText, X } from "lucide-react";
...
export const CartSummaryModal = ({ isOpen, onClose }: CartSummaryModalProps) => {
  ...
  const handleDecrement = (item: any) => {
    const newQuantity = Math.round((item.quantity - item.step) * 1000) / 1000;
    if (newQuantity < item.minQuantity) return; // Do not go below min
    const newPrice = newQuantity * item.pricePerUnit;
    updateQuantity(item.id, newQuantity, newPrice);
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white p-0 gap-0 overflow-hidden shadow-2xl border-none">
        ...
        <div className="bg-slate-900 p-6 text-white text-center relative">
          ...
          <DialogTitle className="text-xl font-serif tracking-wide">VOTRE COMMANDE</DialogTitle>
          <p className="text-white/60 text-xs mt-1 uppercase tracking-widest">Boucherie Artisanale</p>
        </div>

        {/* Paper jagged edge effect (Decorative) */}
              ...
              {items.map((item) => (
                <div key={item.id} className="flex flex-col gap-2 border-b border-dashed border-gray-200 pb-4 last:border-0">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    <div>
                      {formatPrice(item.pricePerUnit)} / {item.unitType}
                    </div>
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-200">
           <div className="flex justify-between items-end mb-6">
              <span className="font-bold text-gray-500 text-sm uppercase">Total à Payer</span>
              <span className="font-bold text-primary text-2xl font-mono">{formatPrice(cartTotal())}</span>
           </div>

           <div className="grid grid-cols-3 gap-3">
               <Button variant="outline" onClick={clearCart} className="col-span-1 border-red-100 text-red-500 hover:bg-red-50 hover:text-red-600">
                  Vider
               </Button>
               <Button className="col-span-2 bg-primary hover:bg-red-700 text-white shadow-lg font-bold">
                  Commander
               </Button>
           </div>
        </div>
