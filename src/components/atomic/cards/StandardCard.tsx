...
interface StandardCardProps extends Product {
  ...
  // animationDelay est conservé dans le type pour compatibilité, mais n'est plus utilisé pour le style
  animationDelay?: number; 
}

export const StandardCard = (props: StandardCardProps) => {
  // On extrait les props, mais on ignore animationDelay car c'est le parent (Index.tsx) qui gère l'apparition
  const { onAdd, animationDelay, ...product } = props;
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-50">
        <ImageWithFallback
          src={product.image}
          alt={product.title}
          // --- OPTIMISATIONS CRITIQUES POUR LA FLUIDITÉ ---
          loading="lazy"   // Ne charge l'image que si elle est visible
          decoding="async" // Décode l'image en arrière-plan sans bloquer le scroll
          // -----------------------------------------------
        ...
        <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
           <div>
             <p className="text-xs text-gray-400 font-medium">
               Prix au {product.unitType === 'kg' ? 'kilo' : 'pièce'}
             </p>
